    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                corrected_exp_avg_sq = exp_avg_sq.div(bias_correction2).add_(group['eps'])

                #### Steady state Bayesian filtering
                lr_sgd = group['lr'] * group['lr_ratio'] / group['batch_size']

                sigma_2m2 = 0.5/lr_sgd
                eta_m2 = 1/group['lr']**2

                # inverse learning rate
                lpost = corrected_exp_avg_sq.mul_(eta_m2).add_(sigma_2m2**2).sqrt_().add_(sigma_2m2)

                p.data.addcdiv_(-1/bias_correction1, exp_avg, lpost)

                if 0 != group['weight_drift']:
                    p.data.mul_(1-group['weight_drift'])
